...
module.exports.getBudget = async( req,res)=>{
    try{
      const budget =  await getBudget.find();
        res.status(200).json(budget);
    }catch(err){
...
module.exports.getBudgetById = async( req,res)=>{
    ...
    try{
     const budget =  await getBudgetById.findById(id);
       res.status(200).json(budget);
    }catch(err){
...
module.exports.updateBudget = async( req,res)=>{
    const id = req.params.id;
     try{
     const budget =  await updateBudget.findByIdAndUpdate(id);
       res.status(200).json(budget);
    }catch(err){
        res.status(500).json(err.message);
    }
...
module.exports.deleteBudget = async( req,res)=>{
     const id = req.params.id;
     try{
     const budget =  await deleteBudget.findByIdAndDelete(id);
